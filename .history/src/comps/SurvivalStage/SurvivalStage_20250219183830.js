    ...
    const checkCollision = () => {
      setFishList((prevFishList) => {
        ...
        prevFishList.forEach((fish) => {
          ...
          if (distance < 50) {
            console.log(`🦈 Collision detected! Fish at (${fish.x}, ${fish.y}) eaten.`);
            eatenCount++;
          } else {
        ...
        if (eatenCount > 0) {
          setFishEaten((prevCount) => prevCount + eatenCount); // Update fish eaten count
        }

        return newFishList; // Return a new array instead of mutating state
      });
    };
