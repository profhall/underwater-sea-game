...
const SurvivalStage = () => {
  ...
  const [fishSpawned, setFishSpawned] = useState(0); // Track total spawned fish
  ...
  // Spawn fish ONLY until the limit is reached
  useEffect(() => {
  ...
  useEffect(() => {
    const checkCollision = () => {
      setFishList((prevFishList) =>
        prevFishList.filter((fish) => {
          const distance = Math.hypot(fish.x - position.x, fish.y - position.y);
          if (distance < 50) {
            setFishEaten((prevCount) => prevCount + 1); // Increase eaten counter
            return false; // Remove fish from the list
          }
          return true;
        })
      );
