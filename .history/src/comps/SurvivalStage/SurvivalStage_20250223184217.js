    ...
    <>
      ...
      <Stage width={dimensions.width} height={dimensions.height} options={{ backgroundColor: 0x1099bb }}>
        ...
        {fishList.map((fish) => (
          <Fish
          key={fish.key}
          fishInfo={fish}           // Pass the fish object (its key, x, y, size, etc.)
          sharkPosition={position}  // Pass the current shark position
          image={randomFishImage}
          initialPosition={{ x: fish.x, y: fish.y }}aimport React, { useState, useEffect } from 'react';
          import { Sprite, Stage } from '@pixi/react';
          import MovingSprite from '../Sprites/MovingFish/MovingFish';
          import Fish from '../Sprites/Fish/Fish';
          
          const MAX_FISH_COUNT = 10; // Exactly 10 fish
          ...
          const SurvivalStage = () => {
            ...
            // Spawn exactly 10 fish when the game starts (only once)
            useEffect(() => {
              if (gameStarted) {
                const newFishList = [];
            ...
            // Collision detection between shark and fish remains the same
            useEffect(() => {
              const checkCollision = () => {
                console.log(`🦈 Shark position: (${position.x}, ${position.y})`); // Track shark movement
                  ...
                  {fishList.map((fish) => (
                    <Fish
                      ...
                      onEaten={() => {
                        ...
                        setFishEaten((prevCount) => prevCount + 1);
                      }}
                    />
                  ))}
          ...
          onEaten={() => {
            console.log(`❌ Removing fish at (${fish.x}, ${fish.y}) from fishList`);
            setFishList((prevFishList) => prevFishList.filter((f) => f.key !== fish.key));
           setFishEaten((prevCount) => prevCount + 1); // Update the counter here too
          }}
        />
        ))}
